...
<head>
  ...
  <link rel="stylesheet" href="<?php echo $this->public_url('libs/bootstrap-chosen-master/bootstrap-chosen.css') ?>">
      ...
      $(function(){
          $('#calendar').fullCalendar({
              ...
              eventClick: function(calEvent, jsEvent, view) {
                  ...
                  $.ajax({
                      url: '<?php echo $this->base_url('employee/check_schedule') ?>',
                      method: 'post',
                      data:{id:id},
                      dataType: 'json',
                      success: function(data){
                          if(data == 'success'){
                              ...
                              $('.btn-view-item').click(function(){
                                  var id = calEvent.id;
                                  swal.close();
                                  window.location.href = '<?php echo $this->base_url('employee/schedule_detail/') ?>'+id ;
                              });
      ...
      function list_employee(){
          $.ajax({
              ...
              success: function(data){
                  // console.log(data);
                  let html = '';
                  let option = '<option value="<?php echo $_SESSION['emp']->emp_id ?>" selected>Me</option>';
                  // let option = '';
                  if (data[0] == 'success') {
                      let department = '';
                      let check_close = '';
