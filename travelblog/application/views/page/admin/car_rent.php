		...
		<div class="modal-dialog modal-dialog-centered bd-example-modal-x" role="document" style="max-width:1200px">
			<div class="modal-content modal-xl">
				<div class="modal-header">
					<h3 class="modal-title text-dark" id="exampleModalCenterTitle">เพิ่มรถเช่า</h3>
					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="modal-body">
					<form class="" action="<?php echo $this->base_url('admin/add_car_rent') ?>" method="post" enctype="multipart/form-data">
						<div class="row pl-5 pr-5">
							<div class="col-md-12 pb-2">
								<label for="">ชื่อรถเช่า:</label>
								<input type="text" class="form-control input-place-name" name="car_rent_name" value="" required>
							</div>
							<div class="col-md-12 pb-2">
								<label for="">รายละเอียด:</label>
								<textarea name="car_rent_detail" class="form-control" rows="5" cols="80" required></textarea>
							</div>
	...
	<div class="modal fade" id="modal_edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
		<div class="modal-dialog modal-dialog-centered bd-example-modal-x" role="document" style="max-width:1200px">
			<div class="modal-content modal-xl">
				...
				<div class="modal-body">
					<form class="" action="<?php echo $this->base_url('admin/update_car_rent') ?>" method="post" enctype="multipart/form-data">
						<input type="hidden" name="car_rent_id" value="">
						<div class="row pl-5 pr-5">
							<div class="col-md-12 pb-2">
								<label for="">ชื่อรถเช่า:</label>
								<input type="text" class="form-control input-place-name" name="car_rent_name" value="" required>
							...
							<div class="col-md-12 pb-2">
								<label for="">รายละเอียด:</label>
								<textarea name="car_rent_detail" class="form-control" rows="5" cols="80"></textarea>
							</div>
							<div class="list-item row"></div>
						</div>
						<div class="row pl-5 pr-5">
						...
						<div class="col fill_height no-padding">
							...
							<div class="search_tabs_container">
								<div class="search_tabs d-flex flex-lg-row flex-column align-items-lg-center align-items-start justify-content-lg-between justify-content-start">
									...
									<a href="<?php echo $this->base_url('admin/car_rent') ?>" class="search_tab active d-flex flex-row align-items-center justify-content-lg-center justify-content-start">รถเช่า</a>
        ...
        <div class="intro">
            <div class="container">
				<div class="row intro-item col-md-6" style="float:unset;margin:auto;margin-bottom:15px;">
					<button type="button" class="btn btn-success btn-add col-md-12" style="" name="button"><i class="fa fa-plus-square" style="margin-right:5px;"></i>เพิ่มรถเช่า</button>
				</div>
				<div class="list-car-rent">
    ...
    $(document).ready(function(){
		...
		.on('click','.btn-delete',function(){
			...
			swal({
				...
				if (result) {
					window.location.href = '<?php echo $this->base_url("admin/delete_car_rent") ?>/'+id+'/car_rent';
				}
			})
		})
		.on('click','.btn-edit',function(){
			...
			$('#modal_edit').find('input[name=car_rent_name]').val(name);
			$('#modal_edit').find('input[name=car_rent_id]').val(id);
			$('#modal_edit').find('textarea[name=car_rent_detail]').val(detail);
					...
					if (data[0] == 'success') {
						...
						$.each(data[1],function(key,value){
							if ((key+1) %2 == 0) {
								...
								html += '<div class="offers_content pl-4">';
								html += '<div class="offers_price text-content" detail="'+value.car_rent_detail+'" shop-name="'+value.car_rent_name+'">';
								html += '<p><b>'+value.car_rent_name+'</b></p>';
								let new_line = value.car_rent_detail;
								new_line = new_line.split('\n');
								$.each(new_line,function(key_line,value_line){
									if (value_line != '') {
