...
<head>
  ...
  <style media="screen">
      ...
      .list-ro .table thead tr th{
          font-size: 12px !important;
      }
      .list-ro .card .table tr td{
          font-size: 12px !important;
      }
      /* table quotation*/
      .list-ro .table thead tr th{
                ...
                <!-- <div class="col-lg-4 col-md-12 list-status">
                    <div class="card">
                        <div class="card-header card-header-warning">
                            <h4 class="card-title">เลือกวันที่แสดง</h4>
                        </div>
              ...
              <div class="card card-equipment-list">
                <div class="card-header row d-flex justify-content-between">
                    ...
                    <div class="card-header-info text-center" style="width: fit-content !important">
                        <h4 class="card-title">ทั้งหมด <b class="num-ro">0</b> รายการ</h4>
                    </div>
                </div>
                <div class="card-body table-responsive">
                    <div class="navbar">
                    ...
                    <table class="table table-hover table-bordered tb-quotation-list">
                        <thead class="text-info">
                            <tr>
                                <th>หมายเลขใบแจ้งซ่อม,ใบงานซ่อม</th>
                                <th>โรงพยาบาล</th>
                                <th>แผนก</th>
                                <th>ชื่อช่าง/เขต</th>
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('.form-default-report').on('click','.btn-default-report',function(key,value){
            ...
            $.ajax({
                url: '<?php echo $this->base_url("Repair_order/report") ?>',
                method: 'post',
                ...
                success: function(data){
                    ...
                    if (data[0] == 'success') {
                        ...
                        // $('.navbar select.search-cate').html(select_search_ro_working_result_type);
                        let html_status = '';
                        ...
                        $('.content .list-ro .card-title b').html(num_order);
                        $('.content .list-ro tbody').html(html);
                        ...
                        let colspan = $('.list-ro .tb-quotation-list thead tr th').length;
                        html += '<tr><td class="text-center" colspan="'+colspan+'">ไม่มีรายการใบแจ้งซ่อม</td></tr>';
                        $('.content .list-ro tbody').html(html);
                        html = '<tr><td class="text-center" colspan="2">ไม่มีรายการใบแจ้งซ่อม</td></tr>';
                        $('.content .list-status tbody').html(html);
                        $('.content .list-ro .card-title b').html('0');
                        // $('.navbar select.search-cate').html(select_search_ro_working_result_type);
                    }
        ...
        function search_sort(data){
            ...
            .on('click','.btn-reset-search',function(){
                ...
                $('.navbar .form-search .search-ro_working_result_type').val('');
                ...
                $.each(data,function(key,value){
                    if (value.ro_status != 'cancel') {
                        ...
                        html += '<td>'+value.ro_customer_department+'</td>';
                        html += '<td>'+value.ro_working_service_name+'</td>';
                        html += '<td>'+value.ro_working_service_head+'</td>';
                        // html += '<td>'+value.ro_working_result_service_name+'</td>';
                ...
                $('.content .list-ro .card-title b').html(num_ro);
                $('.content .list-ro tbody').html(html);
            })
            .on('change keyup','.search-q_number,.search-date-start,.search-date-end,.search-sort,.search-ro_working_result_type,.search-ro_working_result_product_status',function(e){
                ...
                if (cate_work != '' || cate_product_status != '') {
                    if (cate_work != 'cancel') {
                        ...
                        data_key_cate = 0;
                        for (var i = 0; i < data_search.length; i++) {
                            if (data_search[i].ro_working_result_type == cate_work) {
                                data_search_cate[data_key_cate++] = data_search[i];
                            }
                        }
                        if (data_search_cate.length < 0) {
                ...
                if (data_search.length < 1) {
                    ...
                    if (cate_work != '') {
                        // text += '<span class="text-'+cate_work+'">'+cate_text_work+'</span><br>';
                    }
                    if (date_start != '') {
                    ...
                    let colspan = $('.list-ro .tb-quotation-list thead tr th').length;
                    html += '<tr><td colspan="'+colspan+'" class="text-center" colspan="4">ไม่มีรายการใบแจ้งซ่อมตามที่ค้นหา</td></tr>';
                    let number_order = 0;
                    $('.content .list-ro .card-title b').html(number_order);
                    $('.content .list-ro tbody').html(html);
                }else {
                    let check_line_today = 0;
                    let num_ro = 0;
        ...
        function list_ro(){
            $('.content .list-ro tbody').html('<tr><td colspan="20" style="padding-bottom: 160px;"></td></tr>');
            loader($('.content .list-ro tbody tr td'));
            $.ajax({
                url: '<?php echo $this->base_url('Repair_order/report') ?>',
                method: 'post',
                data:{status:'get'},
                    ...
                    if (data[0] == 'success') {
                        ...
                        $.each(data[1],function(key,value){
                            if (value.ro_status != 'cancel') {
                                ...
                                // html += '<td>'+value.ro_working_result_service_name+'</td>';
                                // let ro_status = '';
                                // if (value.ro_working_result_type != '' && value.ro_status != 'cancel') {
                                //     if (value.ro_working_result_product_status != '' && value.ro_working_result_product_status != 'work') {
                                //         if (value.ro_working_result_product_status != 'work' ) {
                                //             switch (value.ro_working_result_type) {
                                //                 case 'work':
                                //                     ro_status = '<span class="text-info">ปิดจ๊อบ</span>';
                                //                     break;
                                //                 case 'get_back':
                                //                     ro_status = '<span class="text-info">ยกเครื่องกลับ</span>';
                                //                     break;
                                //                 case 'quotation':
                                //                     ro_status = '<span class="text-info">เสนอราคา</span>';
                                //                     break;
                                //                 case 'other':
                                //                     ro_status = '<span class="text-warning">อื่นๆ('+value.ro_working_result_detail+')</span>';
                                //                     break;
                                //                 default:
                                //             }
                                //         }else {
                                ...
                                //     }
                                // }else {
                                //     ro_status = value.ro_status;
                                //     switch (ro_status) {
                                //         case 'new':
                                //         ro_status = '<span class="text-info">สร้างใหม่</span>';
                                //         break;
                                //         case 'edit':
                                //         ro_status = '<span class="text-warning">ต้องแก้ไข</span>';
                                //         break;
                                //         case 'cancel':
                                //         ro_status = '<span class="text-danger">ยกเลิก</span>';
                                //         break;
                                //         case 'approved':
                                //         ro_status = '<span class="text-success">หัวหน้าอนุมัติ</span>';
                                //         if (value.q_status_customer != ''){
                                //             switch (value.q_status_customer) {
                                //                 case 'created':
                                //                 q_status += ',<span class="text-danger">สั่งพิมพ์แล้ว</span>';
                        ...
                        //         select_search_ro_working_result_type += '<option value="get_back">ยกเครื่องกลับ</option>';
                        //         select_search_ro_working_result_type += '<option value="other">อื่นๆ</option>';
                        //         select_search_ro_working_result_product_status += '<option value="cancel">ยกเลิก</option>';
                        //         break;
                        //     case 'service':
                        //         select_search_ro_working_result_type += '<option value="new">สร้างใหม่</option>';
                        //         select_search_ro_working_result_type += '<option value="work">ปิดจ็อป</option>';
                        ...
                        //     case 'supervisor':
                        //         select_search_ro_working_result_type += '<option value="new">สร้างใหม่</option>';
                        //         select_search_ro_working_result_type += '<option value="edit">ต้องแก้ไข</option>';
                        //         select_search_ro_working_result_type += '<option value="cancel">ยกเลิก</option>';
                        //         select_search_ro_working_result_type += '<option value="approved">หัวหน้าอนุมัติ</option>';
                        //         select_search_ro_working_result_type += '<option value="customer_approved">ลูกค้าอนุมัติ</option>';
                        //         select_search_ro_working_result_type += '<option value="customer_reject">ลูกค้ายกเลิก</option>';
                        //         break;
                        //     case 'it':
                        //         select_search_ro_working_result_type += '<option value="new">สร้างใหม่</option>';
                        //         select_search_ro_working_result_type += '<option value="edit">ต้องแก้ไข</option>';
                        ...
                        $('.content .list-ro .card-title b.num-ro').html(num_ro);
                        $('.content .list-ro tbody').html(html);
                        let html_status = '';
                        $.each(data[3].status,function(key,value){
                            let q_status = key;
                            switch (q_status) {
                                case 'get_back':
                                    q_status = '<span class="text-info">ยกเครื่องกลับ</span>';
                                    break;
                                case 'work':
                                    q_status = '<span class="text-info">ปิดจ็อป</span>';
                                    break;
                                case 'other':
                                    q_status = '<span class="text-warning">อื่นๆ</span>';
                                    break;
                                default:
                            }
                            html_status += '<tr status="'+key+'">';
